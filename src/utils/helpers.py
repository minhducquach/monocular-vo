...
def compute_relative_scale(camera, map, prev_matches, curr_matches, curr_keypoints, curr_T):
    ...
    f1_to_f0 = {m.queryIdx: m.trainIdx for m in prev_matches}
...
def visualize(map, ground_truth, prev_frame, frame, keypoints, descriptors, matches, id):
    ...
    line_gt.set_data(ground_truth[:len(traj_np), 0, 3], ground_truth[:len(traj_np), 2, 3])
